...
public class InfiniteReach extends Module {
    ...
    private final Setting<Boolean> onlyMace = sgMace.add(
            new BoolSetting.Builder()
                    .name("Only When Mace")
    ...
    public void onActivate() {
        if (Modules.get().get(MaceKill.class).isActive()) error("MaceKill doesn't work well with InfiniteReach. Consider disabling it.");
    }
    @EventHandler
    ...
    private boolean isValidTeleport(Vec3d pos) {
        ...
        for (Entity e : mc.world.getOtherEntities(mc.player, box)) {
            if (e.isCollidable(mc.player)) return false;
        }
    ...
    private void moveTo(Vec3d pos) {
        ...
        if (mc.player.getVehicle() != null) {
            ...
            mc.getNetworkHandler().sendPacket(
                    new VehicleMoveC2SPacket(pos, mc.player.getYaw(), mc.player.getPitch(), false)
            );
        } else {
            mc.player.setPosition(pos);
            mc.getNetworkHandler().sendPacket(
                    new PlayerMoveC2SPacket.PositionAndOnGround(pos.x, pos.y, pos.z, false, false)
            );
        }
    }
