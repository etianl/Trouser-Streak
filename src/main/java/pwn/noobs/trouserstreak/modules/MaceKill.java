...
public class MaceKill extends Module {
    ...
    private void onSendPacket(PacketEvent.Send event) {
        if (mc.player != null
            ...
            && event.packet instanceof IPlayerInteractEntityC2SPacket packet
                    ...
                    if (isSafeBlock(isopenair1) && isSafeBlock(isopenair2)) {
                        ...
                        if (blocks <= 22) {
                            if (mc.player.hasVehicle()) {
                                // **UNCHANGED VEHICLE LOGIC (small)**
                                for (int i = 0; i < 4; i++) {
                                    mc.player.networkHandler.sendPacket(
                                        VehicleMoveC2SPacket.fromVehicle(mc.player.getVehicle())
                                    );
                                }
                                double maxHeight = Math.min(
                                    mc.player.getVehicle().getY() + 22,
                                ...
                                // **PATCHED PLAYER LOGIC (both small & large)**
                                for (int i = 0; i < 4; i++) {
                                    mc.player.networkHandler.sendPacket(
