...
public class HoleAndTunnelAndStairs_Finder extends Module {
    ...
    private final Setting<Integer> staircaseHeight = sgParams.add(new IntSetting.Builder()
            .name("Staircase Height")
            .description("Height of the staircase to be detected")
            .defaultValue(3)
            .min(2)
            .sliderMax(5)
            .build()
    ...
    public HoleAndTunnelAndStairs_Finder() {
        ...
        for (Direction dir : DIRECTIONS) {
            ...
            while (isStaircaseSection(currentPos, dir)) {
                Box stairsBox = new Box(
                        ...
                        currentPos.getZ(),
                        currentPos.getX() + 1,
                        currentPos.getY() + staircaseHeight.get(),
                        currentPos.getZ() + 1
                );
    ...
    private boolean isStaircaseSection(BlockPos pos, Direction dir) {
        int height = staircaseHeight.get();
        for (int i = 0; i < height; i++) {
            if (!isPassableBlock(pos.up(i))) return false;
        }
        if (isPassableBlock(pos.down()) || isPassableBlock(pos.up(height))) return false;
    ...
    private boolean isPassableBlock(BlockPos pos) {
        ...
        else return state.getCollisionShape(mc.world, pos).isEmpty();
    }
