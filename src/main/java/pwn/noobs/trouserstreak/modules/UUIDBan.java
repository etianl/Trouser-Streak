...
public class UUIDBan extends Module {
    ...
    public void onTick(TickEvent.Post event) {
        ...
        if (mc.player.getPermissionLevel() < requiredOpLevel.get()) {
        ...
        if (uuidCollectionMode.get() == uuidCollectionModes.AllPlayers) {
            if (mc.getNetworkHandler() != null) {
                for (PlayerListEntry entry : playerlistattimeofkick) {
                    ...
                    String summonCommand = String.format("summon villager ~ ~ ~ " + makeEntityNBT(entry.getProfile().getId()));
    ...
    private void onMouseButton(MouseButtonEvent event) {
        ...
        if (clickBan.get() && mc.options.attackKey.isPressed() && mc.player.getPermissionLevel() >= requiredOpLevel.get()) {
            runBanCommands();
        }
        ...
        if (uuidCollectionMode.get() == uuidCollectionModes.AllPlayers) {
            if (mc.getNetworkHandler() != null) {
                ...
                for (PlayerListEntry entry : playerlistattimeofkick) {
                    ...
                    entityName = entry.getProfile().getName();
                    runBanCommand();
                }
                ticksLeft = delay.get();
            }
