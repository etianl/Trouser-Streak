    ...
    @Override
        ...
        builder.executes(ctx -> {
            if (mc.player.getBlockY()>64) {
                ...
                ChatUtils.sendMsg(Component.nullToEmpty("Lava will take " + time + " seconds to go from your elevation (Y" + TopY + ") to Y64(sea level) on a 45degree staircase at 20TPS)."));
            } else if (mc.player.getBlockY()<=64) {
                float TopY = mc.player.getBlockY();
                float time = (((TopY - (-60)) * 60) / 20);

                ChatUtils.sendMsg(Component.nullToEmpty("Lava will take " + time + " seconds to go from your elevation (Y" + TopY + ") to Y-60(Bottom of the world) on a 45degree staircase at 20TPS)."));
            }
        ...
        builder.then(argument("TopY", FloatArgumentType.floatArg()).executes(ctx -> {
            if (mc.player.getBlockY()>64) {
                ...
                ChatUtils.sendMsg(Component.nullToEmpty("Lava will take " + time + " seconds to go from Y" + TopY + " to Y64(sea level) on a 45degree staircase at 20TPS)."));
            } else if (mc.player.getBlockY()<=64) {
                float TopY = FloatArgumentType.getFloat(ctx, "TopY");
                float time = (((TopY - (-60)) * 60) / 20);

                ChatUtils.sendMsg(Component.nullToEmpty("Lava will take " + time + " seconds to go from Y" + TopY + " to Y-60(Bottom of the world) on a 45degree staircase at 20TPS)."));
            }
            return SINGLE_SUCCESS;
        }));
        builder.then(argument("TopY", FloatArgumentType.floatArg()).then(argument("BottomY",FloatArgumentType.floatArg()).executes(ctx -> {
            ...
            float BottomY = FloatArgumentType.getFloat(ctx, "BottomY");
                ...
                ChatUtils.sendMsg(Component.nullToEmpty("Lava will take "+time+" seconds to go from  Y"+TopY+" to Y"+BottomY+" on a 45degree staircase at 20TPS)."));
                return SINGLE_SUCCESS;
            })));
        builder.then(literal("lastmountain").executes(ctx -> {
            if (((((2+new AutoMountain().highestblock.getY()-new AutoMountain().lowestblock.getY())*60)/20)+(((new AutoMountain().lowestblock.getY()-new AutoMountain().groundY)*30)/20)) <= (((((2+new AutoMountain().highestblock.getY()-new AutoMountain().lowestblock.getY())*60)/20)/2)+(((new AutoMountain().highestblock.getY()-new AutoMountain().groundY2)*30)/20))){
                ...
                }else ChatUtils.sendMsg(Component.nullToEmpty("Lava will take "+time+" seconds to flow to the ground across your last Mountain"));
            }
